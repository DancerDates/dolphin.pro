...
//--- AQB Searchable fields ---//
function getViewableValue( $aItem, $sValue ) {
			global $site;
			$value = $this->getViewableValue_default($aItem, $sValue);
			if (strlen($value) && (!isset($aItem['Searchable']) || $aItem['Searchable']) && in_array($aItem['Type'], array('text', 'num', 'date', 'range', 'bool', 'select_one', 'select_set'))) {
				if ($aItem['Type'] == 'select_set') {
					$value = $this->getViewableSelectSetInLinks($aItem, $sValue);
				} else {
					$sValue = addslashes(urlencode($sValue));
					$value = "<a href=\"{$site['url']}search.php?field={$aItem['Name']}&value={$sValue}\">{$value}</a>";
				}
			}
			return $value;
		}
		function getViewableSelectSetInLinks( $aItem, $sValue, $sUseLKey = 'LKey' ) {
			global $aPreValues;
			global $site;

			$mValues = $aItem['Values'];
			...
			if( is_string($mValues) and substr($mValues, 0, 2) == '#!' ) {
				...
				foreach( $aValues as $sValue ) {
					$sValueRaw = addslashes(urlencode($sValue));
					$aTValues[] = "<a href=\"{$site['url']}search.php?field={$aItem['Name']}&list_item={$sValueRaw}\">".htmlspecialchars_adv(_t( $aPreValues[$sKey][$sValue][$sUseLKey] ))."</a>";
				}

				return implode( ', ', $aTValues );
			} elseif( is_array($mValues) ) {
				$aValues = array();
				foreach( explode( ',', $sValue ) as $sValueOne ) {
					$sValueRaw = addslashes(urlencode($sValueOne));
					$aValues[] = "<a href=\"{$site['url']}search.php?field={$aItem['Name']}&list_item={$sValueRaw}\">".htmlspecialchars_adv(_t( "_FieldValues_{$sValueOne}" ))."</a>";
				}
...
//--- AQB Searchable fields ---//
function getViewableValue_default( $aItem, $sValue ) //--- AQB Searchable fields ---//
    {
        global $site;
