...
class BxWmapModule extends BxDolModule
{
    ...
    function serviceLocationBlock($sPart, $iEntryId)
    {
        ...
        if ($r && !empty($r['lat'])) {
            ...
            //*********************************Navigational Links Section (Jason)*********
            ...
            $iMaxLink = 'https://www.google.com/maps/place/';
            ...
            $iMaxLink .= $r['lat'].",".$r['lng']."/@".$r['lat'].",".$r['lng'].",285m"."/data=!3m1!1e3"; 	// satellite view then zoom and centre map
            
            
            // geodistance addition
            $iDistance = BxDolService::call('geo_locator', 'GetMyDistance', array($iEntryId,$sPart));
            if ($iDistance == "")
                {
                    $iDistance = "No distance info";
                }
            //****************************************************************************************************
            
            $aVars2 = array(
                'map' => $this->_oTemplate->parseHtmlByName('map', $aVars),
                'MapFullScreen' => $iMaxLink,
                'NavigateMe' => $iNavLink,                
                'distance' =>$iDistance,            
            );
