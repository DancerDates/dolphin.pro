...
$resultsal = mysqli_query($query);
$contasal = mysqli_num_rows($resultsal);
...
$resultreqfriends = mysqli_query($query);
$contareqamicizie = mysqli_num_rows($resultreqfriends);
...
if ($showfriendsreq=="ON")
{
 if ($contareqamicizie>0) 
 {
  ...
  {
   $richiestaamiciaz=mysqli_fetch_array($resultreqfriends);
...
if ($contaamici>0)
{
 ...
 {
  $listaamici=mysqli_fetch_array($resultfriends);
...
//ottengo gli id degli utenti che ho bloccato
 ...
 $getblocked=mysqli_query($getidblocked);
 $numblocked=mysqli_num_rows($getblocked);
 if ($numblocked>0)
 {
  for ($d=0;$d<$numblocked;$d++)
  {
   $getidbl=mysqli_fetch_array($getblocked);
   $idblk=$getidbl[0];
 ...
 $runsuggested=mysqli_query($getidsuggested);
 $numalreadysugg= mysqli_num_rows($runsuggested);
 if ($numalreadysugg>0)
 {  
  for ($c=0;$c<$numalreadysugg;$c++)
  {
   $getidsg=mysqli_fetch_array($runsuggested);
   $idsg=$getidsg[0];
 ...
 //verifico che l'utente non abbia superato il max numero di richieste giornaliere
  ...
  $resultche = mysqli_query($queryverificache);
  $allora=mysqli_fetch_array($resultche);
 ...
 $resultsugge = mysqli_query($querysugge);
 $contasugge = mysqli_num_rows($resultsugge); 
 ...
 if ($contasugge>0)
 {
  ...
  if ($allora[2]>$allora[1])
  {
   ...
   $lanciarest = mysqli_query($queryrest);	
  }
  ...
  if ($allora[0]<$maxnumberoffriendrequests)
  {
   ...
   {
    $listasugge=mysqli_fetch_array($resultsugge);
    ...
    if ((($conditionto=="OR") AND (($metchpercentage>$trsugg) or ($mutualfriends>$trfriends))) OR (($conditionto=="AND") AND (($metchpercentage>$trsugg) and ($mutualfriends>$trfriends))))
    {
 	   ...
 	   $resultqueryinserta = mysqli_query($queryinserta);
    }
 ...
 if ($allora[0]<$maxnumberoffriendrequests)
 {
   ...
   $resultsceltaa = mysqli_query($selectsuggestion);
   $contasscelta = mysqli_num_rows($resultsceltaa);
   ...
   {
    $listadeiscelti=mysqli_fetch_array($resultsceltaa);  
    ...
    { 
     ...
     $removeprofile="DELETE FROM suggerimenti WHERE (FriendID=".$infosugge['ID']." AND mioID=".$userid.") OR (mioID=".$infosugge['ID']." AND FriendID=".$userid.")";
	   $execdeleteprofile=mysqli_query($removeprofile);
    }
    if ($infosugge['NickName']=="") 
	  {//rimozione profili non esistenti
	   ...
	   $execdeleteprofile=mysqli_query($removeprofilecanceled);
	  }
    else
    {
