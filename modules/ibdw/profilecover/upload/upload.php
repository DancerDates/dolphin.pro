...
$querynamefile=title2uri($temptitle);
    ...
    $instquery = "INSERT INTO bx_photos_main (ID,Owner,Ext,Size,Title,Uri,Status,Hash,bx_photos_main.Date) VALUES
                                              ('$lastid','$userid','$ext','$size','$temptitle','$querynamefile','approved','$hash',".time().")";
    $runquery = mysql_query($instquery);
    
    $instquery = "INSERT INTO sys_albums_objects (id_album,id_object) VALUES('$idalbum','$lastid')";
    $runquery = mysql_query($instquery);
    ...
    $verifica = "SELECT ID FROM ibdw_profile_cover WHERE Owner = ".$userid." LIMIT 1";
    $exeverifica = mysql_query($verifica);
    $num_rows = mysql_num_rows($exeverifica);
    
    if($num_rows==0){ 
    $instquery = "INSERT INTO ibdw_profile_cover (Owner,Hash) VALUES('".$userid."','".$hash."')";
    $runquery = mysql_query($instquery);
    }
    else { 
    ...
    $instquery = "INSERT INTO bx_spy_data (sender_id,lang_key,params) VALUES('".$userid."','".$key."','".$str."')";
    $runquery = mysql_query($instquery);
unlink($_POST['file']);
