<?php    
  ...
  $idalbum = (int)$_POST['id_album_predef']; 
  $mioid = (int)$_POST['user'];
  $modulepath="modules/ibdw/profilecover/";
  $text_too_big=str_replace("AMOUNTMB",$maxfilesize,_t("_ibdw_profilecover_max_size"));  
?> 
<div class="covercontainer">
 <div id="upboxtitle"><?php echo _t('_ibdw_profilecover_uploadtitle');?></div>
 <div id="upboxdisc"><?php echo _t('_ibdw_profilecover_uploaddesc');?></div>
<div onclick="closeuploader();stopmn=0;" class="exit_alb"><i class="sys-icon remove"></i></div>
    ...
    <button id="ok" onclick="$('#loading_div').css('display','block');update_profilecore();" class="bx-btn bx-btn-small bx-btn-ifont"><i class="sys-icon chevron-right"></i><span><?php echo _t("_ibdw_profilecover_formcontinue");?></span></button>
    <button id="no" onclick="cancel_profilecore();stopmn=0;" class="bx-btn bx-btn-small bx-btn-ifont"><i class="sys-icon remove"></i><span><?php echo _t("_ibdw_profilecover_cancelupload");?></span></button>
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
    <tr class="template-upload fade">
        ...
        <td>
            <p class="size"><?php echo _t('_ibdw_profilecover_processing');?></p>        
        </td>
        <td>
            {% if (!i && !o.options.autoUpload) { %}
                <button class="btn btn-primary start" disabled>
                    <i class="sys-icon upload"></i>
                    <span><?php echo _t('_ibdw_profilecover_start');?></span>
                </button>
            {% } %}
            {% if (!i) { %}
                <button class="btn btn-warning cancel">
                    <i class="sys-icon remove"></i>
                    <span><?php echo _t('_ibdw_profilecover_cancelupload');?></span>
                </button>
            {% } %}
        </td>
    </tr>
...
$(function () {
    ...
    $('#fileupload').fileupload({
        ...
        url: 'modules/ibdw/profilecover/temp/',
        maxFileSize: <?php echo $maxfilesize*1048576;?>,
        messages: {
                maxNumberOfFiles: '<?php echo _t("_ibdw_profilecover_max_num_file");?>',
                acceptFileTypes: '<?php echo _t("_ibdw_profilecover_wrongtype");?>',
                maxFileSize: '<?php echo $text_too_big;?>',
                minFileSize: '<?php echo _t("_ibdw_profilecover_file_too_small");?>'
            },
...
function update_block(){
  $.ajax({
      type: "POST",
      data: "ajax2=1",
      url: "modules/ibdw/profilecover/core.php",
      success: function(data) {
        $("#pfblockconteiner").html(data);
      }
  });
}
function cancel_profilecore()
{
 $("#modificaalbums").css("display","none");
 var r = confirm("<?php echo _t('_ibdw_profilecover_delete_confirm');?>")
    if(r == true)
    { 
        $.ajax({
          ...
          error: function (data) {
          //$('#pfblockconteiner').append(data);
             //alert("<?php echo _t('_ibdw_profilecover_failed_delete');?>");
          }
        });
    }
}
function update_profilecore()
{
 $.ajax({        
          type: "POST",
          url: '<?php echo BX_DOL_URL_ROOT.$modulepath."upload/upload.php";?>',
          data: {'file' : "<?php echo BX_DIRECTORY_PATH_ROOT.$modulepath . 'temp/files/'?>" + filenameis, 'tempfile' : "<?php echo BX_DIRECTORY_PATH_ROOT.$modulepath . 'temp/files/thumbnail/'?>" + filenameis, 'user' : '<?php echo $mioid;?>','album':'<?php echo $idalbum;?>','filename' : filenameis},
          success: function (data) {
          ...
          error: function (data) {
             //alert("<?php echo _t('_ibdw_profilecover_failed_upload');?>");
          }
        });
}
</script>
