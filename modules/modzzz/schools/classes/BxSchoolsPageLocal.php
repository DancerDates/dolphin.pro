...
class BxSchoolsPageLocal extends BxDolTwigPageMain {
 
    function __construct(&$oMain, $sCountry, $sState) {
		...
		if($sCountry)
			parent::__construct('modzzz_schools_local_state', $oMain); 
		else	
			parent::__construct('modzzz_schools_local', $oMain); 

	}
 
    function getBlockCode_StateSchools() {
 
		if($this->sState){ 
			return $this->ajaxBrowse('local_state', $this->oDb->getParam('modzzz_schools_perpage_main_recent'), array(), $this->sState);
		}elseif($this->sCountry){
			return $this->ajaxBrowse('local_country', $this->oDb->getParam('modzzz_schools_perpage_main_recent'), array(), $this->sCountry);
		}
	}
    ...
    function getBlockCode_States() {
	    ...
	    $aStates = array($this->oTemplate->parseHtmlByName('school_states', $aVars)); 
		$aStates[3] = _t('_modzzz_schools_browse_schools_in_country', $sCountryName);
    ...
    function getBlockCode_Region() {
	    ...
	    return $this->oTemplate->parseHtmlByName('school_regions', $aVars);  
	}
