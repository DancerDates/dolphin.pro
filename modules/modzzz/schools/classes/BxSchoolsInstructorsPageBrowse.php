...
class BxSchoolsInstructorsPageBrowse extends BxDolTwigPageMain {
    ...
    function __construct(&$oMain, $sUri) {
        ...
        $this->sUrlStart = BX_DOL_URL_ROOT . $this->oConfig->getBaseUri() .  'instructors/browse/' . $this->sUri;
        $this->sUrlStart .= (false === strpos($this->sUrlStart, '?') ? '?' : '&');  

        $this->sSearchResultClassName = 'BxSchoolsSearchResult';
        $this->sFilterName = 'filter';
		parent::__construct('modzzz_schools_instructors_browse', $oMain);
	}
