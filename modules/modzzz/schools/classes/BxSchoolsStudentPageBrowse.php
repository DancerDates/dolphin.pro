...
class BxSchoolsStudentPageBrowse extends BxDolTwigPageMain {
    ...
    function __construct(&$oMain, $sUri) {
        ...
        $this->sUrlStart = BX_DOL_URL_ROOT . $this->oConfig->getBaseUri() .  'student/browse/' . $this->sUri;
        $this->sUrlStart .= (false === strpos($this->sUrlStart, '?') ? '?' : '&');  

        $this->sSearchResultClassName = 'BxSchoolsSearchResult';
        $this->sFilterName = 'filter';
		parent::__construct('modzzz_schools_student_browse', $oMain);
	}
