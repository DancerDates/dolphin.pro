...
class BxSchoolsInstructorsCmts extends BxTemplCmtsView {
    ...
    function isPostReplyAllowed () {
        if (!parent::isPostReplyAllowed())
            return false;
        $oMain = $this->getMain();  
		$aInstructorEntry = $oMain->_oDb->getInstructorsEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableInstructors, $aInstructorEntry);
    }

    function isEditAllowedAll () {
        $oMain = $this->getMain(); 
		$aInstructorEntry = $oMain->_oDb->getInstructorsEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aInstructorEntry['school_id']);
