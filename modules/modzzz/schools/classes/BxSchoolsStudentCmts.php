...
class BxSchoolsStudentCmts extends BxTemplCmtsView {
    ...
    function isPostReplyAllowed () {
        if (!parent::isPostReplyAllowed())
            return false;
        $oMain = $this->getMain();  
		$aStudentEntry = $oMain->_oDb->getStudentEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableStudent, $aStudentEntry);
    }

    function isEditAllowedAll () {
        $oMain = $this->getMain(); 
		$aStudentEntry = $oMain->_oDb->getStudentEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aStudentEntry['school_id']);
