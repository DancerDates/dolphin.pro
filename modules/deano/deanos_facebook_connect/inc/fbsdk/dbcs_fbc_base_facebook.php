 ...
 */
class dbcs_fbc_FacebookApiException extends Exception
{
  /**
 ...
 * @author Naitik Shah <naitik@facebook.com>
 */
abstract class dbcs_fbc_BaseFacebook
{
  /**
   * Version.
   */
   ...
   * The cookie is not set by the dbcs_fbc_BaseFacebook class, but
   ...
   * metadata. The cookie is not set by the dbcs_fbc_BaseFacebook class, but it may be
   ...
   * @throws dbcs_fbc_FacebookApiException
   */
  ...
  protected function makeRequest($url, $params, $ch=null) {
    ...
    if (curl_errno($ch) == 60) { // CURLE_SSL_CACERT
      ...
      curl_setopt($ch, CURLOPT_CAINFO,
                  dirname(__FILE__) . '/dbcs_fbc_fb_ca_chain_bundle.crt');
      $result = curl_exec($ch);
    }
    ...
    if ($result === false) {
      $e = new dbcs_fbc_FacebookApiException(array(
  ...
  protected function throwAPIException($result) {
    $e = new dbcs_fbc_FacebookApiException($result);
    switch ($e->getType()) {
      // OAuth 2.0 Draft 00 style
      case 'OAuthException':
        // OAuth 2.0 Draft 10 style
  ...
  /**
   * Get the data for $key, persisted by dbcs_fbc_BaseFacebook::setPersistentData()
   *
