...
$iLoggedId = _getLoggedId();
if($iLoggedId) {
  $sOverPath = BX_DIRECTORY_PATH_ROOT . 'modules/andrew/tm4/images/' . $iLoggedId . '/';
  if (! file_exists($sOverPath)) {
    //@mkdir($sOverPath); // win
    @mkdir($sOverPath, 0777); // unix
  }
}

	$config['img_path'] = 'modules/andrew/tm4/images/' . $iLoggedId . '/'; // Relative to domain name
	// $config['upload_path'] = $_SERVER['DOCUMENT_ROOT'] . $config['img_path']; // Physical path. [Usually works fine like this]
	$config['upload_path'] = $sOverPath;
