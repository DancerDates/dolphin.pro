...
class BxBaseProfileGenerator extends BxDolProfile
{
    ...
    */
	...
	//--- AQB Searchable fields ---//
		if( $_REQUEST['field'] ) {
			$field = process_db_input($_REQUEST['field']);
			$value = process_db_input($_REQUEST['value']);
			$list_item = process_db_input($_REQUEST['list_item']);
			if (strlen($value)) $aWhere[] = "`{$field}` = '{$value}'";
			else $aWhere[] = "FIND_IN_SET('{$list_item}', `{$field}`)";
		}
	//--- AQB Searchable fields ---//
        // add online only
